...
//CEI comment start//
#define DIFFERENCE_HWOCV_SWOCV		25	/* 105% difference */
//CEI comment end//
#endif

#ifndef DIFFERENCE_SWOCV_RTC
//CEI comment start//
#define DIFFERENCE_SWOCV_RTC		30	/* 10% difference */
//CEI comment end//
#endif
...
int g_battery_id_voltage[] = {1420, 1611, -1}; //initially sync to ap build and later
...
//CEI comment start// //Check every migration//
	bm_print(BM_LOG_CRTI, "[force_get_tbat]LE(K)=> rbat_pull_up_volt=%d, dwVolt=%d, TRes=%lld, g_rbat_pull_up_r=%d\n", RBAT_PULL_UP_VOLT, dwVolt, TRes, g_rbat_pull_up_r);
//CEI comment end//
...
//CEI comment start//
	bm_print(BM_LOG_CRTI, "LE(K)=> [force_get_tbat] id_voltage_read=%d, read_fail_flag=%d, range_incorrect=%d, retry_counter=%d, battery_id = %d\n", id_voltage_read, id_volt_read_fail_flag, id_volt_range_incorrect, batt_id_retry_counter, g_fg_battery_id);
//CEI comment end//
...
//CEI comment start//
//Since MTK can't get Ichg for EXTERNAL_SWCHR_SUPPORT case, we use this node to monitor Ibat (Charging)
#if 0 // MTK original 
	return 0;
#else
	signed int ret = 0;
...
//	TRes = (RBAT_PULL_UP_R * dwVolt) / (RBAT_PULL_UP_VOLT - dwVolt);
	TRes = (g_rbat_pull_up_r* dwVolt) / (g_rbat_pull_up_r - dwVolt);
//CEI comment end//
...
static int battery_meter_dts_probe(struct platform_device *dev)
{
	...
	//Solve VBIF power doesn't init issue in L, M is solved but keep the code body
	//init_vbif28_pmu();
	//CEI comment end//
